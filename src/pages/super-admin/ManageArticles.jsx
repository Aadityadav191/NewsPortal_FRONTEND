...
const ManageArticles = () => {
  ...
  const [selectedArticle, setSelectedArticle] = useState(null); // For detail preview modal
  ...
  const handleAction = async (article, status) => {
    ...
    try {
      ...
      if (selectedArticle && (selectedArticle.slug || selectedArticle.id || selectedArticle._id) === slugOrId) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-2 sm:p-4">
      ...
      {loading ? (
        ...
        /* Empty State */
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center max-w-xl mx-auto my-12">
          <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Sparkles className="w-8 h-8" />
          </div>
          <h3 className="text-xl font-bold text-gray-900 mb-1">No Pending Articles</h3>
          <p className="text-gray-500 text-sm mb-6 max-w-sm mx-auto">
            There are no pending articles awaiting approval right now. New submissions will appear here.
          </p>
          <button
            onClick={fetchArticles}
            className="px-5 py-2.5 bg-gray-900 text-white font-medium text-xs rounded-xl hover:bg-gray-800 transition-colors shadow-sm"
          >
            Refresh 
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {pendingArticles.map((article) => {
            ...
            const isApproving =
              processingId?.id === articleKey && processingId?.status === "APPROVED";
            const isRejecting =
              processingId?.id === articleKey && processingId?.status === "REJECTED";
              ...
              >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-xs text-gray-500 font-medium">
                      <div className="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1 rounded-md border border-gray-100">
                        ...
                        <span className="text-gray-700 truncate max-w-37.5">{authorName}</span>
                      </div>
                      {article.createdAt && (
                ...
                <div className="px-6 pb-6 pt-2 space-y-3">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-2 text-xs font-semibold text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors border border-gray-100"
                  >
                  ...
                  <div className="flex gap-3 pt-2 border-t border-gray-100">
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-semibold py-2.5 rounded-xl text-xs transition-colors shadow-xs disabled:opacity-50"
                    >
                      {isApproving ? (
                    ...
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-1.5 bg-rose-50 hover:bg-rose-100 active:bg-rose-200 text-rose-600 font-semibold py-2.5 rounded-xl text-xs transition-colors border border-rose-100 disabled:opacity-50"
                    >
                      {isRejecting ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
      ...
      {selectedArticle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl border border-gray-100 animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            ...
            <div className="p-6 sm:p-8 overflow-y-auto space-y-4">
              ...
              <p className="text-xs text-gray-500 font-medium">
                ...
                <span className="text-gray-800 font-semibold">
                  {selectedArticle.authorName || selectedArticle.author?.name || "Author"}
                </span>
              </p>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50/50 flex gap-3">
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2.5 rounded-xl text-xs transition-colors"
              >
                <Check className="w-4 h-4" /> Approve Article
              </button>
              <button
                onClick={() => handleAction(selectedArticle, "REJECTED")}
                className="flex-1 inline-flex items-center justify-center gap-2 bg-rose-50 hover:bg-rose-100 text-rose-600 font-semibold py-2.5 rounded-xl text-xs transition-colors border border-rose-100"
              >
